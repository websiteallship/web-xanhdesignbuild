...
const XanhPortfolioDetail = {
  ...
  _buildLightboxThumbs(lightbox, slides) {
    ...
    slides.forEach((s, i) => {
      ...
      btn.className = 'ba-lb-thumb' + (i === 0 ? ' is-active' : '');
      btn.type = 'button';
      btn.dataset.index = i;
      btn.setAttribute('aria-label', s.title);

      const imgWrap = document.createElement('div');
      imgWrap.className = 'ba-lb-thumb__img';
      ...
      nameSpan.className = 'ba-lb-thumb__name';
      nameSpan.textContent = s.title.split(' — ')[0].split(' & ')[0];

      const bar = document.createElement('div');
      bar.className = 'ba-lb-thumb__bar';
  ...
  _initGalleryLightbox(section) {
    ...
    const closeBtn2 = document.getElementById('gallery-lb-close');
    const backdrop2 = lightbox.querySelector('.gallery-lb__backdrop');
    const prevBtn2 = document.getElementById('gallery-lb-prev');
    const nextBtn2 = document.getElementById('gallery-lb-next');
    if (closeBtn2) closeBtn2.addEventListener('click', close);
    if (backdrop2) backdrop2.addEventListener('click', close);
    if (prevBtn2) prevBtn2.addEventListener('click', () => goTo(currentIndex - 1));
    if (nextBtn2) nextBtn2.addEventListener('click', () => goTo(currentIndex + 1));
    document.addEventListener('keydown', (e) => {
      if (!lightbox.classList.contains('is-open')) return;
      if (e.key === 'Escape') close();
      if (e.key === 'ArrowLeft') goTo(currentIndex - 1);
  ...
  _initGalleryAnimations(section) {
    ...
    /* gsap.registerPlugin is handled once in main.js */
    const defaults = typeof ANIM_DEFAULTS !== 'undefined'
      ? ANIM_DEFAULTS.fadeUp
      : { y: 40, duration: 0.8, ease: 'power2.out' };
    const stagger = typeof ANIM_DEFAULTS !== 'undefined' ? ANIM_DEFAULTS.stagger : 0.1;
      ...
      if (typeof gsap !== 'undefined') {
        ...
        gsap.fromTo(hiddenItems,
          { opacity: 0, y: 20 },
          {
            ...
            y: 0,
            duration: 0.5,
            ease: (typeof ANIM_DEFAULTS !== 'undefined') ? ANIM_DEFAULTS.fadeUp.ease : 'power2.out',
            stagger: 0.06,
          }
        );
      }
