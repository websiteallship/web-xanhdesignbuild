...
/**
 * Auto-populate defaults for "Service Cards" repeater.
 */
function xanh_services_items_defaults( $value, $post_id, $field ) {
	$defaults = [
		[
	...
	foreach ( $value as $i => &$row ) {
		...
		foreach ( $defaults[ $i ] as $key => $default_val ) {
			if ( empty( $row[ $key ] ) ) { $row[ $key ] = $default_val; }
		}
	}
	return $value;
}
add_filter( 'acf/load_value/key=field_hp_services_items', 'xanh_services_items_defaults', 10, 3 );

/**
 * Auto-populate defaults for "Portfolio Projects" repeater.
 */
function xanh_portfolio_projects_defaults( $value, $post_id, $field ) {
	$defaults = [
		[
			'tag'      => 'NHÀ PHỐ',
			'title'    => 'Villa Bình An',
			...
			'quote'    => '"Xanh đã biến giấc mơ của chúng tôi thành hiện thực — từ bản vẽ đến ngày trao chìa khóa, mọi thứ đều hoàn hảo hơn mong đợi."',
			'author'   => '— Anh Minh Tuấn, Chủ nhà',
		],
		[
			'tag'      => 'BIỆT THỰ',
			...
			'quote'    => '"Đội ngũ XANH không chỉ xây nhà — họ kiến tạo không gian sống gắn liền với thiên nhiên."',
			'author'   => '— Chị Thu Hương, Chủ nhà',
		],
		[
			'tag'      => 'CĂN HỘ',
			...
			'quote'    => '"Mỗi chi tiết đều được chăm chút tỉ mỉ, từ vật liệu đến màu sắc. Chúng tôi rất hài lòng."',
			'author'   => '— Anh Đức Anh, Chủ nhà',
		],
		[
			'tag'      => 'CẢI TẠO',
			...
			'area'     => '240 m²',
			'duration' => '6 tháng',
			'type'     => 'Nhà phố cải tạo',
			'quote'    => '"Ngôi nhà 30 năm tuổi giờ đây mang diện mạo hoàn toàn mới mà vẫn giữ được hồn cốt gia đình."',
			'author'   => '— Chị Lan Anh, Chủ nhà',
		],
	];
...
add_filter( 'acf/load_value/key=field_hp_portfolio_projects', 'xanh_portfolio_projects_defaults', 10, 3 );

/**
 * Auto-populate defaults for "Process Steps" repeater.
 */
function xanh_process_steps_defaults( $value, $post_id, $field ) {
	$defaults = [
		[
			'num'   => '01',
			'title' => 'Tư Vấn',
...
add_filter( 'acf/load_value/key=field_hp_process_steps', 'xanh_process_steps_defaults', 10, 3 );

/**
 * Auto-populate defaults for "CTA Contact" text fields.
 */
function xanh_ctac_eyebrow_default( $value, $post_id, $field ) {
	return $value ? $value : 'Bắt Đầu Hành Trình';
}
add_filter( 'acf/load_value/key=field_hp_ctac_eyebrow', 'xanh_ctac_eyebrow_default', 10, 3 );

function xanh_ctac_heading_default( $value, $post_id, $field ) {
	return $value ? $value : 'Mọi Tổ Ấm Bình Yên<br>Đều Bắt Đầu Từ<br>Một Cuộc Trò Chuyện.';
}
add_filter( 'acf/load_value/key=field_hp_ctac_heading', 'xanh_ctac_heading_default', 10, 3 );

function xanh_ctac_body_default( $value, $post_id, $field ) {
	return $value ? $value : 'Hãy để XANH lắng nghe câu chuyện của bạn — đồng hành từ những ý tưởng sơ khai nhất đến ngày trao tay chiếc chìa khóa.';
}
add_filter( 'acf/load_value/key=field_hp_ctac_body', 'xanh_ctac_body_default', 10, 3 );

function xanh_ctac_btn_text_default( $value, $post_id, $field ) {
	return $value ? $value : 'Đặt Lịch Tư Vấn Riêng';
}
add_filter( 'acf/load_value/key=field_hp_ctac_btn_text', 'xanh_ctac_btn_text_default', 10, 3 );

/**
 * Auto-populate defaults for "Partners" repeater.
 */
function xanh_partners_items_defaults( $value, $post_id, $field ) {
	$defaults = [
		[ 'name' => 'Dulux' ],
		[ 'name' => 'An Cường' ],
...
add_filter( 'acf/load_value/key=field_hp_partners_items', 'xanh_partners_items_defaults', 10, 3 );
