...
function xanh_ai_decrypt_key( string $encrypted ): string {
    ...
    // If stored value already looks like a plaintext API key, return directly.
    if ( str_starts_with( $encrypted, 'AIza' ) ) {
        return $encrypted;
    }
    ...
    // Validate decrypted result looks like a Google API key.
    if ( false === $result || ! str_starts_with( $result, 'AIza' ) ) {
        return '';
    }
...
### Lưu ý (cập nhật 2026-03-24)
- `decrypt_key()` có **validation**: kết quả giải mã phải bắt đầu bằng `AIza` (Google API key prefix)
- Nếu giải mã thất bại (vd: `wp_salt` thay đổi), trả về `''` thay vì blob mã hoá
- Hỗ trợ fallback: nếu stored value là plaintext key (bắt đầu `AIza`), trả về trực tiếp
